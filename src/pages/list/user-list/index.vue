<template>
    <div>
        <div class="i-layout-page-header">
            <PageHeader title="用户列表" content="包含用户信息的列表，带有常规操作。" hidden-breadcrumb />
        </div>
            ...
            <List item-layout="vertical">
                <ListItem v-for="(item, index) in data" :key="index">
                    ...
                    <div>
                        <p>
                            ID：{{ item.id }}
                            <Button size="small" icon="md-copy" @click="handleCopyId(item)">复制</Button>
                        </p>
                        <p>
                            邮箱地址：{{ item.mail }}
                        </p>
